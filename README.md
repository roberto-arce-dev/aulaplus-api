...
Sistema escolar con estudiantes y profesores
...
```env
# MongoDB
MONGODB_URI=mongodb://localhost:27017/aulaplus_db
...
```env
MONGODB_URI=mongodb+srv://usuario:password@cluster.mongodb.net/aulaplus_db
...
El servidor estará disponible en: `http://localhost:3015`
...
**Rol asociado:** `PROFESOR`

**Endpoints disponibles:**

#### Obtener mi perfil
```bash
GET /api/profesor-profile/me
Authorization: Bearer {token}
```
...
#### Listar todos los perfiles (Admin)
```bash
GET /api/profesor-profile
Authorization: Bearer {token_admin}
```

#### Obtener perfil por userId (Admin)
```bash
GET /api/profesor-profile/{userId}
Authorization: Bearer {token_admin}
```
...
2. **Reemplazar** `{{id}}` en la URL con el ID copiado:
   ```
   http://localhost:3015/api/curso/507f1f77bcf86cd799439011/upload-image
   ```
...
**Imagen original:**
```
http://localhost:3015/uploads/1700000000000-imagen.jpg
```

**Thumbnail (miniatura):**
```
http://localhost:3015/uploads/thumbnails/thumb-1700000000000-imagen.jpg
```
...
TOKEN=$(curl -X POST http://localhost:3015/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"productor@example.com","password":"password123"}' \
  | jq -r '.access_token')

# 2. Crear Curso
PRODUCTO_ID=$(curl -X POST http://localhost:3015/api/curso \
...
# 3. Subir imagen
curl -X POST http://localhost:3015/api/curso/$PRODUCTO_ID/upload-image \
  -H "Authorization: Bearer $TOKEN" \
...
- La URL debe empezar con `http://localhost:3015/uploads/`
...
```bash
curl -X GET http://localhost:3015/api/estudiante-profile/me \
...
```bash
curl -X PUT http://localhost:3015/api/estudiante-profile/me \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json" \
  -d '{
...
**Archivo:** `aulaplus-api.postman_collection.json`
...
│   │
│   ├── profesor-profile/  # Profile PROFESOR
│   │   ├── dto/
│   │   ├── schemas/
│   │   ├── profesor-profile.controller.ts
│   │   ├── profesor-profile.service.ts
│   │   └── profesor-profile.module.ts
│   │
...
├── aulaplus-api.postman_collection.json
└── package.json
```
