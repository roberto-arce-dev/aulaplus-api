...
export class EstudianteService {
  ...
  async findAll(): Promise<Estudiante[]> {
    const estudiantes = await this.estudianteModel.find().populate('usuario', 'nombre email');
    return estudiantes;
  }

  async findOne(id: string | number): Promise<Estudiante> {
    const estudiante = await this.estudianteModel.findById(id).populate('usuario', 'nombre email');
    if (!estudiante) {
  ...
  async update(id: string | number, updateEstudianteDto: UpdateEstudianteDto): Promise<Estudiante> {
    const estudiante = await this.estudianteModel.findByIdAndUpdate(id, updateEstudianteDto, { new: true }).populate('usuario', 'nombre email')
